...
def get_player(discord_id: str, year: int) -> Optional[Player]:
    conn = get_connection()
    row = conn.execute(
        "SELECT * FROM players WHERE discord_id = ? AND year = ?",
        (discord_id, year)
    ).fetchone()
    conn.close()
...
def register_player(discord_id: str, year: int) -> Player:
    conn = get_connection()
    cursor = conn.execute(
        "INSERT INTO players (discord_id, year) VALUES (?, ?)",
        (discord_id, year)
    )
...
def get_bets_for_player(player_id: int) -> list[Bet]:
    conn = get_connection()
    rows = conn.execute(
        "SELECT * FROM bets WHERE player_id = ? ORDER BY placed_at DESC",
        (player_id,)
    ).fetchall()
    conn.close()
...
def create_bet(
    ...
    cursor = conn.execute(
        """INSERT INTO bets 
           (player_id, platform, market_id, market_title, position, price_cents, placed_year, placed_month)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
        (player_id, platform, market_id, market_title, position, price_cents, now.year, now.month)
    )
